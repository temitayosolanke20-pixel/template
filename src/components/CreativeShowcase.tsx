...
const VIDEOS = [
  { id: 1, src: "/videos/creative-01.mp4", label: "Brand Spot",      tag: "Video Creative" },
  { id: 2, src: "/videos/creative-02.mp4", label: "Product Feature",  tag: "Video Creative" },
  { id: 3, src: "/videos/creative-03.mp4", label: "Campaign Ad",     tag: "Video Creative" },
];
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-4 z-10">
        <span className="text-[9px] font-black tracking-[0.35em] uppercase text-purple-400/80">{video.tag}</span>
      </div>
      <div className="absolute bottom-0 left-0 right-0 p-5 z-10 flex items-end justify-between">
        <p className="text-xs font-black tracking-[0.2em] uppercase text-[#F8F8FF]/80 group-hover:text-[#F8F8FF] transition-colors">
          {video.label}
        </p>
        <button
          onClick={(e) => { e.stopPropagation(); toggleMute(); }}
          aria-label={muted ? "Unmute video" : "Mute video"}
          className="flex items-center justify-center w-8 h-8 rounded-full bg-black/50 border border-white/15 hover:border-purple-500/50 text-[#F8F8FF]/60 hover:text-[#F8F8FF] transition-all duration-300 backdrop-blur-sm opacity-0 group-hover:opacity-100"
        >
          {muted ? (
